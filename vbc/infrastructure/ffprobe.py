...
class FFprobeAdapter:
    ...
    @classmethod
    def _packet_timeline_duration(
        cls,
        packets: list[Dict[str, Any]],
        video_stream_index: Any,
    ) -> float:
        """Return the normalized primary-video timeline length."""
        try:
            expected_index = int(video_stream_index)
        except (TypeError, ValueError):
            expected_index = None
        ...
        for packet in packets:
            ...
            if expected_index is not None:
                try:
                    if int(packet_index) != expected_index:
                        continue
                except (TypeError, ValueError):
                    continue
    ...
    def get_part_info(
        ...
        file_path: Path,
        *,
        scan_packet_timeline: bool = True,
    ) -> Dict[str, Any]:
        """Probe one media file, optionally deriving duration from packet timestamps."""
        cmd = [
            "ffprobe",
            "-v", "error",
            "-print_format", "json",
            ...
            cmd.extend(
                [
                    ...
                    (
                        ...
                        "bit_rate,color_space,pix_fmt,nb_read_packets:"
                        "format=start_time,duration,bit_rate"
                    ),
                ]
            )
        ...
        if scan_packet_timeline and video_stream:
            packet_duration = self._packet_timeline_duration(
                data.get("packets", []),
                video_stream.get("index"),
            )
